...
#define JJImageCount 3


@interface DSNewsHeaderView () <UIScrollViewDelegate>
...
@implementation DSNewsHeaderView

+ (instancetype)headerView
{
    return [[[NSBundle mainBundle] loadNibNamed:@"DSNewsHeaderView" owner:nil options:nil] lastObject];
}
...
- (void)awakeFromNib
{
    ...
    for (NSInteger index = 0; index < JJImageCount; index++) {
        ...
        // 设置图片
        NSString *name = [NSString stringWithFormat:@"%ld", index + 1];
    ...
    // 1.1 将最后一张图片放在第0个位置
    UIImageView *lastImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"3"]];
    ...
    // 5.设置pageControl的总页数
    self.pageViews.numberOfPages = JJImageCount;
    
    [self.scrollView setContentOffset:CGPointMake(imageW, 0)];
    
    self.pageViews.currentPage = 0;
...
- (void)nextImage{
    // 1.增加pageControl的页码
    NSInteger page = 0;
    if (self.pageViews.currentPage == JJImageCount) {
        page = 0;
    } else {
        page = self.pageViews.currentPage + 1; // 1
    }
    // 2.计算scrollView滚动的位置
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    self.pageViews.currentPage = --page;
}
