...
+ (void)hide {
    [self.hudWindow.rootViewController performSelector:@selector(hides:) withObject: ^{
...
+ (UIColor *)hudBackgroundColor {
    if (!objc_getAssociatedObject(self, _cmd)) {
        self.hudBackgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"loading.jpg"]];
    }
    return objc_getAssociatedObject(self, _cmd);
}
...
- (void)displayWillUpdateWithDeltaTime:(CFTimeInterval)deltaTime
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
        ...
        switch (self.direction) {
            ...
            {
                self.lengthTop += (WZSnakeLengthIteration * deltaValue / 2.5);
            ...
            {
                self.heightTop += WZSnakeLengthIteration * deltaValue / 2.5;
            ...
            {
                self.lengthBottom += WZSnakeLengthIteration * deltaValue / 2.5;
            ...
            {
                self.heightBottom += WZSnakeLengthIteration * deltaValue / 2.5;
